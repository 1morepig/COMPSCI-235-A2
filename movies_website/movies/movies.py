...
import movies_website.utilities.utilities as utilities
import movies_website.movies.services as services

from movies_website.authentication.authentication import login_required
...
def movies_by_release_year():
    ...
    if len(movies) > 0:
        ...
        if previous_year is not None:
            ...
            prev_movie_url = url_for('movies_bp.movies_by_release_year', year=previous_year)
            first_movie_url = url_for('movies_bp.movies_by_release_year', year=first_movie['year'])

        # There are movies on a subsequent date, so generate URLs for the 'next' and 'last' navigation buttons.
        if next_year is not None:
            next_movie_url = url_for('movies_bp.movies_by_release_year', year=next_year)
            last_movie_url = url_for('movies_bp.movies_by_release_year', year=last_movie['year'])

        # Construct urls for viewing movie reviews and adding reviews.
        for movie in movies:
            movie['view_review_url'] = url_for('movies_bp.movies_by_release_year', year=target_year, view_reviews_for=movie['id'])
...
def movies_by_actor():
    ...
    movie_ids = services.get_movies_ids_for_actor(actor_name, repo.repo_instance)
    ...
    if cursor > 0:
        ...
        prev_movie_url = url_for('movies_bp.movies_by_actor', actor=actor_name, cursor=cursor - movies_per_page)
        first_movie_url = url_for('movies_bp.movies_by_actor', actor=actor_name)

    if cursor + movies_per_page < len(movie_ids):
        # There are further movies, so generate URLs for the 'next' and 'last' navigation buttons.
        next_movie_url = url_for('movies_bp.movies_by_actor', actor=actor_name, cursor=cursor + movies_per_page)

        last_cursor = movies_per_page * int(len(movie_ids) / movies_per_page)
        if len(movie_ids) % movies_per_page == 0:
            last_cursor -= movies_per_page
        last_movie_url = url_for('movies_bp.movies_by_actor', actor=actor_name, cursor=last_cursor)

    # Construct urls for viewing movie reviews and adding reviews.
    for movie in movies:
        movie['view_review_url'] = url_for('movies_bp.movies_by_actor', actor=actor_name, cursor=cursor, view_reviews_for=movie['id'])
        movie['add_review_url'] = url_for('movies_bp.review_on_movie', movie=movie['id'])
    ...
    return render_template(
        'movies/movies.html',
        title='Movies',
        movies_title='Movies actor: ' + actor_name,
        movies=movies,
        selected_movies=utilities.get_selected_movies(),
        director_urls=utilities.get_directors_and_urls(),
        genre_urls=utilities.get_genres_and_urls(),
...
def movies_by_director():
    ...
    # Retrieve movie ids for movies that are tagged with director_name.
    movie_ids = services.get_movies_ids_for_director(director_name, repo.repo_instance)
    ...
    if cursor > 0:
        # There are preceding movies, so generate URLs for the 'previous' and 'first' navigation buttons.
        prev_movie_url = url_for('movies_bp.movies_by_director', director=director_name, cursor=cursor - movies_per_page)
        first_movie_url = url_for('movies_bp.movies_by_director', director=director_name)

    if cursor + movies_per_page < len(movie_ids):
        # There are further movies, so generate URLs for the 'next' and 'last' navigation buttons.
        next_movie_url = url_for('movies_bp.movies_by_director', director=director_name, cursor=cursor + movies_per_page)

        last_cursor = movies_per_page * int(len(movie_ids) / movies_per_page)
        if len(movie_ids) % movies_per_page == 0:
            last_cursor -= movies_per_page
        last_movie_url = url_for('movies_bp.movies_by_director', director=director_name, cursor=last_cursor)

    # Construct urls for viewing movie reviews and adding reviews.
    for movie in movies:
        movie['view_review_url'] = url_for('movies_bp.movies_by_director', director=director_name, cursor=cursor, view_reviews_for=movie['id'])
        movie['add_review_url'] = url_for('movies_bp.review_on_movie', movie=movie['id'])
    ...
    return render_template(
        'movies/movies.html',
        title='Movies',
        movies_title='Movies director: ' + director_name,
        movies=movies,
        selected_movies=utilities.get_selected_movies(),
        director_urls=utilities.get_directors_and_urls(),
        genre_urls=utilities.get_genres_and_urls(),
...
def movies_by_genre():
    ...
    movie_ids = services.get_movies_ids_for_genre(genre_name, repo.repo_instance)
...
@movies_blueprint.route('/search_movies_by_actor', methods=['GET', 'POST'])
def search_movies_by_actor():
    form = SearchForm()
    searched_actor = None
    if form.validate_on_submit():
        searched_actor = form.search_for.data
    all_actors = services.get_all_actors(searched_actor, repo.repo_instance)
    return render_template(
        'movies/search.html',
        title='Search by actor',
        search_by=all_actors,
        form=form,
        selected_movies=utilities.get_selected_movies(),
        ele_urls=utilities.get_actors_and_urls(searched_actor)
    )


@movies_blueprint.route('/search_movies_by_genre', methods=['GET', 'POST'])
def search_movies_by_genre():
    form = SearchForm()
    searched_genre = None
    if form.validate_on_submit():
        searched_genre = form.search_for.data
    all_genres = services.get_all_genres(searched_genre, repo.repo_instance)
    return render_template(
        'movies/search.html',
        title='Search by genre',
        search_by=all_genres,
        form=form,
        selected_movies=utilities.get_selected_movies(),
        ele_urls=utilities.get_genres_and_urls(searched_genre)
    )


@movies_blueprint.route('/search_movies_by_director', methods=['GET', 'POST'])
def search_movies_by_director():
    form = SearchForm()
    searched_director = None
    if form.validate_on_submit():
        searched_director = form.search_for.data
    all_directors = services.get_all_director(searched_director, repo.repo_instance)
    return render_template(
        'movies/search.html',
        title='Search by director',
        search_by=all_directors,
        form=form,
        selected_movies=utilities.get_selected_movies(),
        ele_urls=utilities.get_directors_and_urls(searched_director)
    )
...
def review_on_movie():
    ...
    if form.validate_on_submit():
        ...
        services.add_review(movie_id, form.review.data, username, repo=repo.repo_instance, rating=form.rating.data)
        ...
        return redirect(url_for('movies_bp.movies_by_release_year', year=movie['year'], view_reviews_for=movie_id))
...
class SearchForm(FlaskForm):
    search_for = StringField('Search...(Uppercase matter)', [
        DataRequired()])
